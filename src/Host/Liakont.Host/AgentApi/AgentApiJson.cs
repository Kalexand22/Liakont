...
/// couverts. <c>allowIntegerValues</c> reste à <c>true</c> (défaut) : un nombre demeure accepté en
/// entrée, donc la liaison est rétro-compatible.</para>
...
internal static class AgentApiJson
{
    ...
    {
        // Requête (modèle pivot).
        options.Converters.Add(new JsonStringEnumConverter<OperationCategory>());
        options.Converters.Add(new JsonStringEnumConverter<VatCategory>());
