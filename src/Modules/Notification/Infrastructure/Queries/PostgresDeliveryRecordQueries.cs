...
public sealed class PostgresDeliveryRecordQueries : IDeliveryRecordQueries
{
    ...
    private static DeliveryRecordDto MapDto(dynamic row)
    {
        return new DeliveryRecordDto
        {
            ...
            SentAt = DbTimestamp.ToDateTimeOffset((object)row.sent_at),
            DeliveredAt = DbTimestamp.ToNullableDateTimeOffset((object?)row.delivered_at),
            FailedAt = DbTimestamp.ToNullableDateTimeOffset((object?)row.failed_at),
