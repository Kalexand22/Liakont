...
public sealed class B2cExportMarkingTests
{
    ...
    // la MÊME ventilation (catégorie détaxée G/K) que l'adjudication (le mapping plateforme classe l'honoraire au
    // même régime que le lot) avec une TVA de ligne nulle (détaxé). Seule la construction de l'entrée change ; les
    ...
    private static PivotLineDto BuyerFeeLine(IReadOnlyList<PivotLineDto> lines, PivotBuyerFeeDto fee)
    {
        ...
        return new PivotLineDto(
            ...
            sourceRegimeCodes: lines.Count > 0 ? lines[0].SourceRegimeCodes : ["EXP_HORSUE"],
