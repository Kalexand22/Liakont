...
public sealed partial class DocumentReceivedConsumer : IIntegrationEventConsumer<DocumentReceivedV1>
{
    ...
    public async Task HandleAsync(IntegrationEvent<DocumentReceivedV1> integrationEvent, CancellationToken cancellationToken = default)
    {
        ...
        // 2) Mapping TVA ligne par ligne (Part = Autre — voir CheckTvaMapping).
        var mappingService = services.GetRequiredService<ITvaMappingService>();
        var plan = CheckTvaMapping.BuildPlan(pivot);
        var mapping = await mappingService.MapAsync(companyId.Value, plan.Requests, cancellationToken);

        // 3) Décision de blocage / mise à ReadyToSend.
        string? blockReason;
        ...
        else
        {
            ...
            else
            {
                ...
                var validationResult = await validation.ValidateAsync(
                    new DocumentValidationContext(evaluation.EnrichedDocument!, companyId.Value), cancellationToken);

                if (validationResult.HasBlockingIssue)
                {
                    blockReason = AggregateBlockingIssues(validationResult);
                }
                else
                {
                    blockReason = null;
                    readyMappingVersion = evaluation.MappingVersion;
                }
            }
        }
