...
/// chacune migrée Common + module Signature. Prouve l'isolation cross-BASE (CLAUDE.md n°9) : un webhook ou un
/// compte écrit dans la base d'un tenant n'est jamais visible dans la base de l'autre.
/// </summary>
public sealed class SignatureMultiTenantFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        SignatureDatabaseFixture.RunCommonMigrations(SystemConnectionString);
    ...
    private async Task CreateTenantDatabaseAsync(string tenantId)
    {
        ...
        SignatureDatabaseFixture.RunCommonMigrations(tenantConnStr);
        SignatureDatabaseFixture.RunModuleMigrations(tenantConnStr);
    }
