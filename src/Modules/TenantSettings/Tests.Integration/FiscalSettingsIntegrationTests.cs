...
public sealed class FiscalSettingsIntegrationTests
{
    ...
    public async Task Set_Then_Update_Persists_Values()
    {
        ...
        await handler.Handle(
            new SetFiscalSettingsCommand { VatOnDebits = false, OperationCategory = "Mixte", ReportingFrequency = "Décadaire" },
            CancellationToken.None);
        await handler.Handle(
            new SetFiscalSettingsCommand { VatOnDebits = true, OperationCategory = null, ReportingFrequency = null },
            CancellationToken.None);
