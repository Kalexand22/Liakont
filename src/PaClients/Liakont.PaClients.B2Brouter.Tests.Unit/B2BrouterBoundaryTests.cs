...
/// <summary>
/// Gardes de frontière + encapsulation du plug-in B2Brouter (acceptance PAB01 ; blueprint.md §2
/// règle 4 ; module-rules §6 ; CLAUDE.md n°6/14/16) : le plug-in ne référence QUE
/// <c>Transmission.Contracts</c> (+ Common), et AUCUN type propriétaire B2Brouter (DTO « fil »,
/// client) n'est exposé hors de l'assembly. Vérifié au niveau IL (réflexion + NetArchTest) ET
/// déclaratif (scan du .csproj).
/// </summary>
public sealed class B2BrouterBoundaryTests
{
    private static readonly Assembly PluginAssembly = typeof(B2BrouterClientFactory).Assembly;
    ...
    public void Plugin_Csproj_Only_Declares_Transmission_Contracts_As_ProjectReference()
    {
        ...
        var csproj = Path.Combine(pluginRoot, "Liakont.PaClients.B2Brouter.csproj");
    ...
    [Fact]
    public void No_Proprietary_B2Brouter_Type_Is_Exposed_Outside_The_Assembly()
    {
        var exported = PluginAssembly.GetExportedTypes().Select(t => t.FullName ?? string.Empty).ToArray();

        exported.Should().NotContain(
            name => name.StartsWith("Liakont.PaClients.B2Brouter.Wire", StringComparison.Ordinal),
            "aucun DTO « fil » B2Brouter (payload propriétaire) n'est exposé hors de l'assembly (acceptance PAB01)");

        exported.Should().NotContain(
            "Liakont.PaClients.B2Brouter.B2BrouterClient",
    ...
    private static string FindPluginRoot()
    {
        ...
        while (dir != null)
        {
            ...
            if (File.Exists(sln))
            {
                return Path.Combine(dir.FullName, "src", "PaClients", "Liakont.PaClients.B2Brouter");
            }
