...
/// <summary>
/// Gardes de frontière + encapsulation du plug-in Super PDP (acceptance PAS02 ; blueprint.md §2 règle 4 ;
/// module-rules §6 ; CLAUDE.md n°6/14/16) : le plug-in ne référence QUE <c>Transmission.Contracts</c>
/// (+ Common), et AUCUN type propriétaire Super PDP (DTO « fil », client OAuth) n'est exposé hors de
/// l'assembly. Vérifié au niveau IL (réflexion + NetArchTest) ET déclaratif (scan du .csproj).
/// </summary>
public sealed class SuperPdpBoundaryTests
{
    private static readonly Assembly PluginAssembly = typeof(SuperPdpClientFactory).Assembly;
    ...
    public void Plugin_Csproj_Only_Declares_Transmission_Contracts_As_ProjectReference()
    {
        ...
        var csproj = Path.Combine(pluginRoot, "Liakont.PaClients.SuperPdp.csproj");
    ...
    [Fact]
    public void No_Proprietary_SuperPdp_Type_Is_Exposed_Outside_The_Assembly()
    {
        var exported = PluginAssembly.GetExportedTypes().Select(t => t.FullName ?? string.Empty).ToArray();

        exported.Should().NotContain(
            name => name.StartsWith("Liakont.PaClients.SuperPdp.Wire", StringComparison.Ordinal),
            "aucun DTO « fil » Super PDP (payload propriétaire) n'est exposé hors de l'assembly (acceptance PAS02)");

        exported.Should().NotContain(
            "Liakont.PaClients.SuperPdp.SuperPdpClient",
            "l'implémentation d'IPaClient reste interne, exposée seulement derrière l'abstraction");

        exported.Should().NotContain(
            "Liakont.PaClients.SuperPdp.SuperPdpTokenProvider",
            "le client OAuth reste interne — aucun type OAuth ne fuit hors de l'assembly (F14 §7)");
    }
    ...
    private static string FindPluginRoot()
    {
        ...
        while (dir != null)
        {
            ...
            if (File.Exists(sln))
            {
                return Path.Combine(dir.FullName, "src", "PaClients", "Liakont.PaClients.SuperPdp");
            }
