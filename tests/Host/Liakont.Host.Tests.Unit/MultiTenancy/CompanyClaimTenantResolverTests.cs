...
/// dérive le tenant du claim <c>company_id</c> du jeton, via <see cref="ICompanyTenantLookup"/>.
/// </summary>
public sealed class CompanyClaimTenantResolverTests
{
    ...
    public void Resolve_Returns_Null_And_Skips_Lookup_When_No_CompanyId_Claim()
    {
        ...
        resolver.Resolve().Should().BeNull();
        lookup.WasQueried.Should().BeFalse("sans claim company_id (non authentifié / chemin agent) on ne touche pas la base");
    }
    ...
    public void Resolve_Returns_Null_And_Skips_Lookup_When_CompanyId_Claim_Is_Malformed()
    {
        ...
        resolver.Resolve().Should().BeNull();
        lookup.WasQueried.Should().BeFalse("un claim non-Guid ne déclenche aucune requête");
    }
    ...
    {
        var lookup = new RecordingLookup(KnownCompanyId, "default");
        var resolver = new CompanyClaimTenantResolver(new HttpContextAccessor { HttpContext = null }, lookup);

        resolver.Resolve().Should().BeNull();
        lookup.WasQueried.Should().BeFalse();
    }
    ...
    private static CompanyClaimTenantResolver BuildResolver(ICompanyTenantLookup lookup, string? claim)
    {
        ...
        return new CompanyClaimTenantResolver(accessor, lookup);
    }
