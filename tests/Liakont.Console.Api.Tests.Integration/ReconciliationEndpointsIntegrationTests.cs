...
public sealed class ReconciliationEndpointsIntegrationTests
{
    ...
    public async Task Reconciliation_Is_Tenant_Scoped()
    {
        ...
        using var client = _factory.CreateClient(ConsoleApiFactory.TenantA, ConsoleApiFactory.ActionsUserId);
        var queue = await GetQueueAsync(client);
    ...
    private HttpClient ActionsClient() =>
        _factory.CreateClient(ConsoleApiFactory.TenantApi04, ConsoleApiFactory.ActionsUserId);
