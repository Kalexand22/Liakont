...
public sealed class ConsoleApiFactory : IAsyncLifetime, IAsyncDisposable
{
    ...
    /// <summary>Compte les événements d'un type donné dans la piste d'audit d'un document (assertion de journalisation).</summary>
    public async Task<int> CountDocumentEventsAsync(string tenant, Guid documentId, string eventType, CancellationToken ct = default)
    {
        ...
        await conn.OpenAsync(ct);
        return await conn.ExecuteScalarAsync<int>(new CommandDefinition(
            "SELECT COUNT(*) FROM documents.document_events WHERE document_id = @Id AND event_type = @Type",
            new { Id = documentId, Type = eventType },
            cancellationToken: ct));
    }
