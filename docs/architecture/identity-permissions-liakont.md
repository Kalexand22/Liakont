...
> de flotte cross-**instance** sur l'instance mutualisée. Elle est portée par un rôle de realm dédié
> à l'exploitant (ex. `fleet`/`exploitant`), distinct des rôles `lecture`/`operateur`/`parametrage`/
> `superviseur` ci-dessus — un utilisateur éditeur ne la reçoit jamais. La télémétrie de flotte est
