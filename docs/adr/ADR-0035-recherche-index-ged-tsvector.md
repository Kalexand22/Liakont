...
- **Nature** : cet ADR **précède** le chantier d'implémentation (module `Liakont.Modules.Ged` non démarré,
  ...
  **dérive de** la conception `docs/conception/F19-GED-Dynamique-Coffre-Fort.md` (statut « proposition NON RATIFIÉE »)
  ...
  il **dépend** d'ADR-0033 (option C / `IGenericArchiveService`, GED07) **seulement** pour ce qui touche l'intégrité du paquet à
  l'affichage d'une fiche (re-lecture `_ged/…` vs `content_hash`, V1). Cet ADR ne tranche **que** la couche « retrouver » : recherche multidimensionnelle, plein
...
colonne dérivée mêlée à un **pivot mutable tracé par change-log** (seules les 3 tables de liens sont append-only-par-chaînage).
F19 §6.1/§6.3 tranche le **foyer unique** : la table dérivée
...
- **Plein texte** (`websearch_to_tsquery('french', @q)`, pondération `setweight` titre=A / axes searchables=B) : le plein
  texte **EXCLUT les axes confidentiels** du `search_vector` partagé en V1 (F19 §6.5), de sorte que le FTS ne fige
  **aucune** valeur confidentielle (cf. INV-GED-07). **Asymétrie ASSUMÉE** : le droit `liakont.ged.confidential`
  **N'OUVRE PAS** l'accès FTS en V1 (un FTS paramétré par le droit serait un index séparé, fast-follow) — à la différence
  de la recherche par axe, la facette et le graphe, qui matérialisent le prédicat `(is_confidential = false OR
  @hasConfidentialRight)` dans le SQL.
...
défaut **4** ; jamais infinie). Cette borne limite la **profondeur**, **pas la largeur** : le résidu de fan-out par nœud
est une **limite ASSUMÉE** en V1 (bornage du nombre de nœuds visités + test de charge = fast-follow), donc **pas une
garde anti-DoS complète** (RL-32) :
...
WITH RECURSIVE reach AS (
    ...
    WHERE  ei.id = @rootEntityId
      AND  (et.is_confidential = false OR @hasConfidentialRight)        -- la RACINE aussi est filtrée (sinon oracle depth-0, RL-31)
  UNION ALL
...
- **Racine filtrée (anti-oracle depth-0, RL-31)** : le prédicat de confidentialité s'applique à la **RACINE** (terme
  d'ancrage) comme aux voisins (terme récursif) : une racine dont l'`entity_type` est confidentiel, sans
  `@hasConfidentialRight`, fait retourner **un ensemble vide** — sinon le canal de fuite se déplace vers le
  `document_entity_links` de la racine (oracle depth-0).
...
- **INV-GED-09** — **Graphe borné + bidirectionnel.** La traversée est **bidirectionnelle**, avec **borne de profondeur
  ...
  pas de colonne `is_confidential`). Le prédicat de confidentialité s'applique aussi à la **racine** (terme d'ancrage de la
  CTE — CANON-A) : **acceptance — racine confidentielle + appelant sans droit ⇒ aucun document retourné (test).** La borne
  de profondeur `maxDepth` borne la **profondeur**, pas la **largeur** : le résidu de largeur (fan-out par nœud) est une
  **limite ASSUMÉE** en V1 (un bornage du nombre de nœuds visités + test de charge est fast-follow), et non une garde
  anti-DoS complète.
- **INV-GED-10** — Confidentialité : masquage server-side sur tous les canaux de restitution. Le prédicat
  `(is_confidential = false OR @hasConfidentialRight)` est MATÉRIALISÉ DANS LE SQL pour la recherche par AXE, la FACETTE et le
  GRAPHE (RL-31, anti-oracle ; racine de graphe incluse — CANON-A). Le PLEIN TEXTE, lui, EXCLUT les axes confidentiels du
  `search_vector` partagé : le droit `liakont.ged.confidential` N'OUVRE PAS l'accès FTS en V1 (asymétrie ASSUMÉE — un FTS
  paramétré par le droit serait un index séparé, fast-follow). L'EXPORT et le LOG masquent/excluent les valeurs confidentielles.
  Le chiffrement au repos reste ❓ NON TRANCHÉ (D9).
...
|---|---|---|---|
| D10 | Volumétrie cible par tenant & **seuil chiffré** de bascule `tsvector → OpenSearch` ; synchronisation index synchrone vs asynchrone | Projection **asynchrone** (latence d'ingestion découplée) ; fixer une volumétrie de référence pour rendre « au volume » vérifiable. **À spécifier** : SLA de **lag de projection** + **alerte de supervision** (réutiliser `Supervision/Application/Rules`, surfacer `refreshed_utc`) ; **bornage du nombre d'axes `searchable`/`facetable`** | Produit + exploitation |
...
- **Poser `search_vector` sur le pivot mutable `managed_documents`** : crée une **double-source non réconciliable**
  (pivot + index) et mêle une colonne dérivée à un **pivot mutable tracé par change-log** (seules les 3 tables de liens
  sont append-only-par-chaînage). **Rejetée** — foyer **unique** `ged_index.document_search`, dérivé reconstructible
  (F19 §6.1/§6.3).
