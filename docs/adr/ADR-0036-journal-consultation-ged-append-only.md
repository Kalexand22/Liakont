...
- `'open_archive'` quand le « lien coffre » **ouvre/atteste** un paquet (ADR-0033 : le lien
  **ouvre/atteste**, ne modifie **jamais** le paquet). L'entrée `'open_archive'` est la **preuve d'accès**
  au paquet scellé ; elle ne déclenche **aucune** écriture sur la chaîne fiscale (`archive_entries`),
  cohérente avec la WORM-neutralité (INV-GED-07).
...
**rétention/minimisation/pseudonymisation** RGPD de `actor_id` (D8), **ni** le **chiffrement au repos** des
...
- **INV-GED-10** — **Confidentialité par masquage server-side** : un axe/`entity_type`/relation
  `is_confidential` est masqué **dans tous les canaux**, log inclus — `query_text` ET `detail` de
  `consultation_log` sont **masqués/hachés** si l'axe ciblé est confidentiel sans le droit
  (`liakont.ged.confidential`) ; le journal **n'est pas un canal de contournement** (anti-oracle). Le
  masquage est **testé en lecture, facette, graphe, export ET log** (F19 §8, ligne « Confidentialité »).
  (Le **chiffrement au repos** des valeurs confidentielles reste D9 ouvert, cohérent règle 10.)
...
(dont `'open_archive'` sans toucher `archive_entries`) ; amendement de `RolePermissionCatalog`
(`liakont.ged.read`/`.export`/`.confidential`) avec test prouvant qu'**aucune** permission GED n'est
...
|---|---|---|---|
| D8 | `consultation_log` : best-effort **ou** fail-closed si finalité probante ; rétention/minimisation RGPD | Best-effort par défaut ; fail-closed **+ alerte** si probant ; minimiser, pseudonymiser `actor_id` après délai (jamais de purge auto en attendant — append-only intégral) | Sécurité + DPO |
