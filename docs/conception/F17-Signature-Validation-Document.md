...
**Conséquence produit.** Le **fait légal est solide** (✅) : **aucune signature électronique (eIDAS) n'est requise** — CGI 289 I-2 ne prescrit **aucune forme** ; la signature (Yousign/Wacom) est donc une **bonne pratique probatoire optionnelle, jamais une obligation** (la coder en gate Blocking « parce que la loi l'exige » violerait CLAUDE.md n°2/3). ⚠️ **EN REVANCHE, la conclusion « tel geste enregistré OUVRE le gate fiscal » n'est PAS normative** tant que l'EC n'a pas validé **quelle forme concrète** qualifie l'« acceptation formelle et expresse » (en particulier l'équivalence accusé-de-réception → acceptation expresse, **réserve EC ci-dessous**). On affirme donc « la signature n'est pas requise » ; on **NE pose PAS** « telle acceptation enregistrée suffit à ouvrir le gate » avant validation EC — **bloquer plutôt qu'ouvrir sur une interprétation non validée** (CLAUDE.md n°3).

> 🔶 **À confirmer EC (et non à présenter comme acquis légal) :** l'équivalence produit « accusé de réception électronique horodaté = acceptation expresse enregistrée (état `Accepted` / capacité `Recorded`) ». **SI** le BOFiP 13/08/2021 admet l'accusé de réception comme modalité d'acceptation (libellé **❓ à revérifier visuellement**, cf. l'encart §1.1 ci-dessus — F15 ne le cite pas), **alors même dans ce cas** en faire un état produit « expresse » resterait une **interprétation produit à valider EC**. Affirmation **non confirmée** à ce stade (cohérence stricte avec le ❓ ci-dessus — à ne pas poser à l'indicatif).
...
> 3. **pour le purpose self-billing, et UNIQUEMENT sur une transition `Validated` EXPRESSE** : la **forme** de l'acceptation expresse est une forme **validée par l'EC** (§1.1). **Tant que l'EC n'a pas tranché, l'arête `Recorded → Validated` du self-billing est NON NORMATIVE et le gate RESTE `Blocked`** — ne jamais transmettre un 389 sur une acceptation dont la forme n'est pas légalement validée (CLAUDE.md n°3 : bloquer plutôt qu'émettre faux). ⚠️ **La condition 3 NE s'applique PAS à `TacitlyValidated`** : l'acceptation **tacite** (mandat écrit + non-contestation au délai) n'est par nature **jamais** « expresse » — elle est régie par ses **propres gardes** (INV-ACCEPT-3 : `EstEcrit = true` **ET** `ContestationDelay` non null **ET** délai écoulé), pas par la forme expresse. Lui appliquer la condition 3 **bloquerait à tort** une acceptation tacite valide (contradiction ADR-0024 / INV-ACCEPT-3).
...
**Hypothèse de conception (À CONFIRMER PAR LE DPO, pas un acquis produit) :** ⚠️ **point de départ — la CNIL liste EXPRESSÉMENT « la dynamique de signature » parmi les techniques biométriques.** Affirmer que « la biométrie seule ne déclenche pas l'AIPD » (au motif que la liste viserait la *reconnaissance* et non la simple *capture*) est une **distinction NON sourcée** : elle **ne peut pas** être posée comme acquis. *Sous réserve de validation DPO* (finalité strictement limitée à la preuve d'intégrité/consentement, **sans gabarit ni identification unique**), la donnée **pourrait** ne pas relever de l'art. 9 (art. 4.14 + considérant 51 — le considérant **n'établit pas** l'exclusion) ; **conclusion strictement ❓ DPO.** **Toute communication commerciale « sans AIPD » est conditionnée à cette validation DPO.**
...
## 10. Points NON TRANCHÉS (avec owner)

| # | Question | Owner |
|---|---|---|
| 1 | Niveau eIDAS (`Recorded`/SES/AES/QES) par besoin (acceptation vs mandat) — aucun texte ne l'impose ; appréciation du risque probatoire, **jamais un défaut produit** | EC |
| 2 | ✅ **DÉCIDÉ (Karl, 2026-06-15) : CHOIX TENANT** — le mandat peut être signé électroniquement OU importé en papier scanné ; `ISignatureProvider` est **optionnel** dans le parcours mandat, jamais imposé | tranché |
| 3 | Confirmer visuellement sur **BOFiP 13/08/2021** l'ancre exacte des §§ et le libellé « acceptation libre / accusé de réception / tacite » ; confirmer art. 224 dir. 2006/112 sur version consolidée | juridique/EC |
| 4 | « Accusé de réception horodaté = acceptation expresse enregistrée (`Recorded`) » : interprétation produit à valider | EC |
| 5 | AES Wacom réellement atteignable (art. 26 a/b/**c**/d) avec SDK + scellement PAdES/CAdES ? Qui détient les données de création / le cert de déchiffrement FSS ? | tech (+ EC : niveau probatoire) |
| 6 | Niveau eIDAS réellement délivré par Yousign selon l'offre, coût/activation AES/QES + vérif identité, rate limits **production**, politique de retry webhooks | tech (+ Karl : coût/offre) |
| 7 | Qualification RGPD exacte de la dynamique (preuve d'intégrité vs identification unique), **validation « hors art. 9 »**, rétention preuve, droit à l'oubli vs conservation fiscale + append-only, périmètre AIPD (y c. couplage données fiscales), **validité du consentement en contexte B2B mandant/mandataire** | juridique/DPO |
| 8 | ✅ **DÉCIDÉ (Karl, 2026-06-15) : le MANDANT signe en personne**, cas **vente aux enchères** (pas criée). Renforce l'art. 26 b (signataire présent/identifié) ; `SignerIdentity` = identification en personne par la SVV au guichet (à spécifier ADR-0030). Reste l'art. 26 c (contrôle exclusif sur pad partagé) à auditer (#5) | tranché |
| 9 | Le 261 de correction ré-entre-t-il dans un cycle d'acceptation (`CreditNoteAcceptance`) ? (F15 §6.5) | EC |
