...
| [F17](F17-Signature-Validation-Document.md) | **Workflow de validation/signature de document réutilisable** : abstraction `ISignatureProvider` à capacités (Yousign à distance + client soft Wacom sur place comme plug-ins), module générique `DocumentApproval` (machine fermée + purposes), intégration `ISelfBilledGate` (ADR-0024) sans modifier sa machine, niveau eIDAS proportionné (recommandation, jamais obligation), secrets par tenant + RGPD biométrie | — (workflow multi-agents sourcé 2026-06-15) | 🟨 note de conception | ✅ légal sourcé primaire (CGI 289 I-2, eIDAS UE 910/2014 art. 25/26, C. civ. 1366/1367, décret 2017-1416) ; **signature NON requise** pour l'acceptation (bonne pratique probatoire) ; ADR-filles 0027-0030 à graver ; points ❓ NON TRANCHÉ (EC/Karl/juridique/DPO/tech) marqués |
| [F18](F18-Plugin-ChorusPro.md) | **Plug-in PA Chorus Pro (dépôt Factur-X B2G via PISTE/OAuth2)** : traçabilité des constantes API/fiscales Chorus Pro AVANT de coder (CLAUDE.md n°2) — double auth (Bearer PISTE `scope=openid` + `cpro-account` base64), `syntaxeFlux = IN_DP_E2_CII_FACTURX`, 9 libellés `etatCourantFlux` + mapping `PaSendState` (`Intégré` → `Issued` seul), endpoints `*.piste.gouv.fr`, `avecSignature=false`, e-reporting exclu (D2) ; transport pur (aucun montant) | — (traçabilité sources, item CP01 2026-06-20) | 🟨 note de conception | 🔶 Spec Externe API Chorus Pro V5.00 (libellés/syntaxes) + raccordement OAuth2 PISTE + décommissionnement URLs (30/09/2023) ; points ❓ À VERROUILLER (base path/codeRetour/cardinalité `idUtilisateurCourant`/acceptation non-signé) tracés ; plan `tasks/plan-chorus-pro.md` |
